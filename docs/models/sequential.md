...
在阅读这片文档前，请先阅读 [Keras Sequential 模型指引](/docs/keras/getting-started/sequential-model-guide)。
...
- __optimizer__: 字符串（优化器名）或者优化器对象。详见 [optimizers](/docs/keras/optimizers)。
- __loss__: 字符串（目标函数名）或目标函数。详见 [losses](/docs/keras/losses)。
...
0 = 安静模式, 1 = 进度条, 2 = 每轮一行。
- __callbacks__: 一系列的 `keras.callbacks.Callback` 实例。一系列可以在训练时使用的回调函数。详见 [callbacks](/docs/keras/callbacks)。
