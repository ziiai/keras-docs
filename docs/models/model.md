...
有关 `Model` 的详细介绍，请阅读 [Keras 函数式 API 指引](/docs/keras/getting-started/functional-api-guide)。
...
详见 [optimizers](/docs/keras/optimizers)。
- __loss__: 字符串（目标函数名）或目标函数。
详见 [losses](/docs/keras/losses)。
...
详见 [callbacks](/docs/keras/callbacks)。
